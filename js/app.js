/* ============================================================
   ...
   Loads sites.json + social.json, handles routing & rendering
   ============================================================ */
...
function cardIconHTML(site) {
  ...
  if (fav) {
    return `<div class="card-icon">
      <img src="${fav}" alt="${site.name} icon"
           onerror="this.parentElement.innerHTML='<span class=\\'icon-fallback\\'>◈</span>'" />
    </div>`;
  }
  return `<div class="card-icon"><span class="icon-fallback">◈</span></div>`;
}
...
function renderHomePage() {
  ...
  document.getElementById('featured-grid').innerHTML =
    siteData.sites.slice(0, 3).map(renderCard).join('');
}
